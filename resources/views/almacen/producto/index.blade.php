                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    @foreach ($producto as $prod)
                                        <tr>
                                            <td>
                                                <a href="{{ route('producto.edit', $prod->id_producto) }}"
                                                    class="btn btn-warning btn-sm"><i class="fas fa-pen"></i>Editar</a>
                                                <button type="button" class="btn btn-danger btn-sm" data-toggle="modal"
                                                    data-target="#modalEliminar{{ $prod->id_producto }}">
                                                    <i class="fas fa-trash">Eliminar</i>
                                                </button>
                                                <button type="button" class="btn btn-success btn-sm" data-toggle="modal"
                                                    data-target="#modalAprobar{{ $prod->id_producto }}">
                                                    <i class="fas fa-check">Aprobar</i>
                                                </button>
                                            </td>
                                            ...
                                            <td>{{ $prod->stock }}</td>
                                            <td><img src="{{ asset('imagenes/productos/' . $prod->imagen_prod) }}"
                                                    alt="{{ $prod->nombre_prod }}" height="70px" width="70px"
                                                    class="img-thumbnail"></td>
                                            @if ($prod->estado == 'Inactivo')
                                                <td class="badge badge-danger">{{ $prod->estado }}</td>
                                            @else
                                                <td class="badge badge-info">{{ $prod->estado }}</td>
                                            @endif
                                        </tr>
                                        @include('almacen.producto.modal')
                                        @include('almacen.producto.modalAprobar')
                                    @endforeach
                                </tbody>
                            </table>
                            {{ $producto->links() }}
